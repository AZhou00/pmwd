...
def force_ray(a_i, a_f, ptcl, ray, cosmo, conf):
    """twirl on rays."""
    twirl, dB = lensing(a_i, a_f, ptcl, ray, cosmo, conf)
    return ray.replace(twirl=twirl, dB=dB)

def kick_ray(ray):
    """Kick."""
    am = ray.am + ray.twirl
    B = ray.B + ray.dB
    return ray.replace(am=am, B=B)
...
def integrate_ray(a_prev, a_next, ptcl, ray, cosmo, conf):
    ...
    for d, k in conf.symp_splits:
        ...
        if k != 0:
            ...
            ray = force_ray(a_vel, a_vel_next, ptcl, ray, cosmo, conf)
            ray = kick_ray(ray)
            a_vel = a_vel_next
    return ray

def observe_init(obsvbl_ray, conf):
    return obsvbl_ray

def nbody_ray_init(a, ray, obsvbl_ray, cosmo, conf):
    obsvbl_ray = observe_init(obsvbl_ray, conf)
...
def nbody_ray(ptcl, ray, obsvbl, obsvbl_ray, cosmo, conf, reverse=True):
    ...
    # does nothing to rays, create observables for each ray
